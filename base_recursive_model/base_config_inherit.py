...
class BaseConfigInheritModel(osv.AbstractModel):
    ...
    """
    Abstract model which can be inherited by model which need to have configuration field. This configuration can
     be inherited by children and specified models, and then be overridden by them.
    """
    ...
    def _get_external_config(self, cr, uid, record, context=None):
        # This function can be overridden in inheriting model to specify how the configuration
        #  can be recovered from other models
        return {}

    def _get_child_ids(self, cr, uid, ids, context=None):
        # This function can be overriden in inheriting model to specify how the children are found
    ...
    def _update_stored_config_external_children(self, cr, uid, ids, context=None):
        # This function can be overriden in inheriting model to trigger the compute of children models
        return True

    def _update_stored_config(self, cr, uid, ids, context=None):
        # When called, this function will purge all stored configuration and recompute them, by recover it from parent
        #  and checking if the configuration isn't override in the current record.
        ...
        config_stored_ids = config_obj.search(
            cr, uid,
            [('model', '=', self._name), ('res_id', 'in', ids), ('stored', '=', True)],
            context=context
        )
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if 'parent_id' in record and record.parent_id:
                parent_config_del_ids = config_del_obj.search(
                    cr, uid,
                    [('model', '=', self._name), ('res_id', '=', record.parent_id.id)],
                    context=context
                )
                ...
                parent_config_ids = config_obj.search(
                    cr, uid,
                    [('model', '=', self._name), ('res_id', '=', record.parent_id.id), ('stored', '=', True)],
                    context=context
                )
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        res = super(BaseConfigInheritModel, self).create(cr, uid, vals, context=context)
        self._update_stored_config(cr, uid, [res], context=context)
        return res

    def write(self, cr, uid, ids, vals, context=None):
        # When we write in the record, we recompute the configuration
        res = super(BaseConfigInheritModel, self).write(cr, uid, ids, vals, context=context)
...
class BaseConfigInheritLine(osv.AbstractModel):
    ...
    """
    Lines containing the configuration. Depending of the stored field, it is either a line computed by the update
     function or a line which override the configuration
    """
...
class BaseConfigInheritLineDel(osv.osv):
    ...
    """
    Lines created when a configuration line where deleted in parent, used to remove the specified configuration from
     children
    """
