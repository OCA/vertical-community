...
class AccountWalletCurrencyLine(orm.Model):
    """
    ...
    def _get_subtotal(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for currency in self.browse(cr, uid, ids, context=context):
            ...
            res[currency.id] = 1.0
            if model_obj.search(
                    cr, uid, [('id', '=', currency.res_id)], context=context
            ):
                transaction = model_obj.browse(
                    cr, uid, currency.res_id, context=context
                )
    ...
    _columns = {
        'model': fields.char(
            'Related Document Model', size=128, required=True, select=1
        ),
        'res_id': fields.integer(
            'Related Document ID', required=True, select=1
        ),
        ...
        'currency_id': fields.many2one(
            ...
            domain=[('wallet_currency', '=', True)], required=True, select=1
        ),
        'subtotal': fields.function(
            _get_subtotal, string='Subtotal',
    ...
    _sql_constraints = [
        (
            'object_currency', 'unique(model,res_id,field,currency_id)',
            'We can only have one currency per record'
        )
    ]
...
class AccountWalletTransaction(orm.Model):
    ...
    The confirm state, used only when there is an external currency
    (currency whose wallet isn't managed in odoo), is used so the receiver
    can confirm that the send gave his the money.
    """

    def _get_price_char(self, cr, uid, ids, prop, unknow_none, context=None):
        # Compute a char from the currency lines,
        # so we can easily display the transaction value in tree view
        res = {}
        for transaction in self.browse(cr, uid, ids, context=context):
            ...
            for currency in transaction.currency_ids:
                if res[transaction.id] != '':
                    res[transaction.id] += ', '
                res[transaction.id] += str(currency.subtotal) + \
    ...
    def _default_currency_ids(self, cr, uid, context=None):
        # When create the transaction, it already contain one
        # line with the default currency
    ...
    def _default_model(self, cr, uid, context=None):
        # Return the model by default, which is account.wallet.transaction.
        # Otherwise, the workflow will be disabled
        proxy = self.pool.get('ir.model.data')
        result = proxy.get_object_reference(
            cr, uid, 'account_wallet', 'model_account_wallet_transaction'
        )
        return result[1]
    ...
    _constraints = [
        (
            _check_same_partner,
            'You cannot make a transaction between the same partner.',
            ['sender_id']
        ),
    ]
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        for transaction in self.browse(cr, uid, ids, context=context):
            ...
            currency_line_obj.unlink(cr, uid, currency_line_ids,
                                     context=context)
        return super(AccountWalletTransaction, self).unlink(
    ...
    def reconcile(self, cr, uid, move_ids, context=None):
        ...
        for account_id, res_account in res.iteritems():
            ...
            for partner_id, res_partner in res_account.iteritems():
                if res_partner['total_debit'] == res_partner['total_credit'] \
                        and account.reconcile:
                    move_line_obj.reconcile(
                        cr, SUPERUSER_ID, res_partner['line_ids'],
                        context=context
                    )
    ...
    def control_amount(
            self, cr, uid, transaction, sender, receiver,
            inv=False, context=None
    ):
        ...
        balance = partner_obj.get_wallet_balance(
            cr, uid, [sender.id]
        )[sender.id]
        for currency in transaction.currency_ids:
            balance_currency = balance[currency.currency_id.id]
            if balance_currency['limit_negative'] \
                    and (balance_currency['available'] - currency.subtotal) \
                    < balance_currency['limit_negative_value']:
                raise osv.except_osv(
                    _('Limit error!'),
                    _("Not enough amount available for %s. Currency : %s, "
                      "Current : %s, Needed : %s, Limit : %s")
                    % (
                        sender.name, currency.currency_id.symbol,
                        balance_currency['available'], currency.subtotal,
                        balance_currency['limit_negative_value']
                    )
                )
        ...
        for currency in transaction.currency_ids:
            balance_currency = balance[currency.currency_id.id]
            if balance_currency['limit_positive'] \
                    and (balance_currency['available'] - currency.subtotal) \
                    > balance_currency['limit_positive_value']:
                raise osv.except_osv(
                    _('Limit error!'),
                    _("Too much amount available for %s. Currency : %s, "
                      "Current %s, Needed : %s, Limit : %s")
                    % (
                        receiver.name, currency.currency_id.symbol,
                        balance_currency['available'], currency.subtotal,
                        balance_currency['limit_positive_value']
                    )
                )
    ...
    def get_account_line(
            self, cr, uid, transaction, action, deduction=0.0,
            inv=False, name='Transaction', context=None
    ):
        ...
        if action == 'reservation':
            self.control_amount(
                cr, uid, transaction, transaction.sender_id,
                transaction.receiver_id, inv=inv, context=context
            )
        ...
        for currency in transaction.currency_ids:
            diff_currency_p = \
                currency.currency_id.id != context['company_currency']
            config_currency_ids = config_currency_obj.search(
                cr, uid, [('currency_id', '=', currency.currency_id.id)],
                context=context
            )
            if not config_currency_ids:
                raise osv.except_osv(
                    _('Config error!'),
                    _("One of the currency is missing in the configuration!")
                )
            config_currency = config_currency_obj.browse(
                cr, uid, config_currency_ids, context=context
            )[0]
            debit_availability_account = \
                config_currency.partner_availability_account_id.id
            debit_reserved_account = \
                config_currency.partner_reserved_account_id.id
            debit_expense_account = \
                config_currency.partner_expense_account_id.id
            credit_availability_account = \
                config_currency.partner_availability_account_id.id
            credit_reserved_account = \
                config_currency.partner_reserved_account_id.id
            credit_income_account = \
                config_currency.partner_income_account_id.id
            ...
            for partner_currency in partner_currency_obj.browse(
                ...
                if 'availability_account' in partner_currency \
                        and partner_currency.availability_account:
                    debit_availability_account = \
                        partner_currency.availability_account.id
                if 'reserved_account' in partner_currency \
                        and partner_currency.reserved_account:
                    debit_reserved_account = \
                        partner_currency.reserved_account.id
                if 'expense_account' in partner_currency \
                        and partner_currency.expense_account:
                    debit_expense_account = \
                        partner_currency.expense_account.id
            ...
            for partner_currency in partner_currency_obj.browse(
                ...
                if 'reserved_account' in partner_currency \
                    ...
                    credit_reserved_account = \
                        partner_currency.reserved_account.id
                if 'income_account' in partner_currency \
                        and partner_currency.income_account:
    ...
    def prepare_move(self, cr, uid, ids, action, context=None):
        ...
        context['company_currency'] = company_obj.browse(
            cr, uid, [company_id]
        )[0].currency_id.id
        for transaction in self.browse(cr, uid, ids, context=context):

            lines = self.get_account_line(
                cr, uid, transaction, action, context=context
            )

            if lines:
                ref = 'Transaction'
                journal_id = config.journal_id.id
                move = move_obj.account_move_prepare(
                    cr, uid, journal_id, date=date, ref=ref
                )
                ...
                self.write(
                    cr, uid, [transaction.id], {action + '_id': move_id}
                )
                if transaction.reservation_id:
                    self.reconcile(
                        cr, uid, [transaction.reservation_id.id, move_id],
                        context=context
                    )
        ...
        for transaction in self.browse(cr, uid, ids):
            ...
            if not skip_confirm:
                workflow.trg_validate(
                    uid, 'account.wallet.transaction', transaction.id,
    ...
    def reset_workflow(self, cr, uid, ids, *args):
        ...
        for transaction in self.browse(cr, uid, ids):
            ...
            workflow.trg_delete(
                uid, 'account.wallet.transaction', transaction.id, cr
            )
            workflow.trg_create(
                uid, 'account.wallet.transaction', transaction.id, cr
            )
...
class ResPartner(orm.Model):
    ...
    def get_wallet_limits(self, cr, uid, ids, currency_ids, context=None):
        ...
        for partner_currency in partner_currency_obj.browse(
            ...
            ][partner_currency.currency_id.id] = {
        ...
        for partner in self.browse(cr, uid, ids, context=context):
            ...
            for currency_id in currency_ids:
                res[partner.id][currency_id] = \
                    config_currency_limits[currency_id]
                if partner.id in partner_currency_limits \
                        and currency_id in partner_currency_limits[partner.id]:
                    res[partner.id][currency_id] = \
                        partner_currency_limits[partner.id][currency_id]
    ...
    def get_wallet_balance(self, cr, uid, ids, context=None):
        ...
        company_obj = self.pool.get('res.company')
        company_id = company_obj._company_default_get(cr, uid)
        company_currency_id = company_obj.browse(
            cr, uid, [company_id]
        )[0].currency_id.id
        ...
        for config_currency in config_currency_obj.browse(
            ...
            default_account[
                config_currency.currency_id.id
        ...
        for partner_currency in partner_currency_obj.browse(
            ...
            partner_currencies[
                partner_currency.currency_id.id
            ][partner_currency.partner_id.id] = accounts

        query = self.pool.get('account.move.line')._query_get(
            cr, uid, context=ctx
        )

        cr.execute("""SELECT l.partner_id, SUM(l.debit-l.credit), l.account_id,
                      l.currency_id, a.code
                      ...
                      AND """ + query + """
                      GROUP BY l.partner_id, l.account_id, l.currency_id,
                      a.code
                      """,
        ...
        limits = self.get_wallet_limits(
            cr, uid, ids, currency_ids, context=context
        )
        ...
        for partner in partners:
            ...
            for currency_id in currency_ids:
                vals = {
                    ...
                    'limit_negative': limits[partner.id][
                        currency_id
                    ]['limit_negative'],
                    'limit_negative_value': limits[partner.id][
                        currency_id
                    ]['limit_negative_value'],
                    'limit_positive': limits[partner.id][
                        currency_id
                    ]['limit_positive'],
                    'limit_positive_value': limits[partner.id][
                        currency_id
                    ]['limit_positive_value']
                }

                if currency_id in partner_currencies \
                        and pid in partner_currencies[currency_id] \
                        and 'available' \
                            in partner_currencies[currency_id][pid] \
                        and partner_currencies[currency_id][pid]['available']:
                    account_id = \
                        partner_currencies[currency_id][pid]['available']
                else:
                ...
                if currency_id in partner_currencies \
                    ...
                    account_id = \
                        partner_currencies[currency_id][pid]['reserved']
                else:
    ...
    def update_wallet_balance(self, cr, uid, ids, context=None):
        ...
        for partner in self.browse(cr, uid, ids, context=context):
            ...
            for currency in balances[partner.id].values():
                if partner.id in lines \
                        and currency['currency_id'] in lines[partner.id]:
    ...
    def write(self, cr, uid, ids, vals, context=None):
        # Trigger an update balance when we make change
        # in currency configuration in partner
    ...
    _columns = {
        'wallet_currency_ids': fields.one2many(
...
class ResPartnerWalletCurrency(orm.Model):
    """
    ...
    _columns = {
        ...
        'available_account': fields.many2one(
            'account.account', 'Available account'
        ),
        'reserved_account': fields.many2one(
            'account.account', 'Reserved account'
        ),
...
class ResPartnerWalletBalance(orm.Model):
    """
    ...
    _columns = {
        ...
        'available': fields.float(
            'Available', digits_compute=dp.get_precision('Product Price')
        ),
        'reserved': fields.float(
            'Reserved', digits_compute=dp.get_precision('Product Price')
        )
    }
    ...
    #     'There was an error when computing the partner balance,
    # tried to create a line while another already exist')
    # ]
