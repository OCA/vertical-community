...
class CommunityConfigSettings(osv.osv):
    ...
    _columns = {
        ...
        'currency_ids': fields.one2many(
            'account.wallet.config.currency', 'config_id', 'Currencies'
        ),
    ...
    # s.pool.get('ir.model.data').get_object(cr, uid,
    # 'account_wallet', 'community_journal').id,
    #     'default_currency_id': lambda s,cr,uid,c:
    # s.pool.get('ir.model.data').get_object(cr, uid,
...
class AccountWalletConfigCurrency(osv.osv):
    ...
    _columns = {
        'config_id': fields.many2one(
            'community.config.settings', 'Config', required=True
        ),
        ...
        'partner_availability_account_id': fields.many2one(
            'account.account', 'Partner Availability account', required=True
        ),
        'partner_reserved_account_id': fields.many2one(
            'account.account', 'Partner Reserved account', required=True
        ),
        'partner_expense_account_id': fields.many2one(
            'account.account', 'Partner Expense account', required=True
        ),
        'partner_income_account_id': fields.many2one(
            'account.account', 'Partner Income account', required=True
        ),
    ...
    _sql_constraints = [
        (
            'currency', 'unique(currency_id)',
            'We can only have one line per currency'
        )
    ]
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        res = super(AccountWalletConfigCurrency, self).create(
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        res = super(AccountWalletConfigCurrency, self).write(
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        res = super(AccountWalletConfigCurrency, self).unlink(
